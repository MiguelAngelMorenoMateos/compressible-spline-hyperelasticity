function [A, y, blockIdx] = build_linear_system_block(model, prot, data, blockNames)
    ...
    for ii = 1:numel(data.modes)
        ...
        Nm = numel(D.P11);
        Pmax = max(abs(D.P11));
        scale1 = sqrt(1/Nm/Pmax);        % For experiment-model fitting in the lsq problems.
        scale2 = 100*sqrt(1/Nm/Pmax);    % For P22=0 BC in the lsq problems.
        ...
        for k = 1:Nm % For all stress-strain data pairs, each corresponding to a raw of A matrix.
            ...
            % Append in same column according to all active optimization parameters from the active blocks (i.e., solve_additive, solve_h, solve_g)
            % dP/dTheta_i obtained as dphi_i/dI1bar dI1bar/dF + ..., for Theta being opt. param. and phi basis functions. Note that dPsi/dTheta_i=phi_i.
            if solve_additive
                if model.use_I1
            ...
            % ---------------- P22 row ----------------
            % A second row for dP22 wrt optimiz. parameters in each column additioanlly appended after the one row for dP11 wrt optimiz. parameters.
            if ~strcmpi(mode,'SS') % The P22=0 condition should be applied only for tension and compression.
                row = row + 1;
            ...
            % Append more rows, as many as experimental stress-strain data pairs.
        end
    end
end